<div>
        ...
        @slot('form')
            <form class="mt-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-12 gap-2">
                    @if ($activity == 'create')
                        <a wire:click='store' wire:loading.attr="disabled" wire:target="store, logo"
                    ...
                    @if ($activity == 'edit')
                        <a wire:click='update' wire:loading.attr="disabled" wire:target="update, logo"
    ...
    <x-table-form>
                    ...
                    <tbody>
                        @foreach ($airlines as $airline)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="py-4 px-6">
                                    <img class="w-10 h-10 rounded" src="{{ Storage::url($airline->logo) }}" alt="{{ $airline->name }}">
                                </td>
                                <td class="py-4 px-6 text-right">
                                    <a wire:click='edit({{ $airline->id }})'
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline cursor-pointer">{{ __('Edit') }}</a>
                                    <a wire:click='modalDelete({{ $airline->id }})'
        ...
        @endslot
        @slot('paginate')
            {{ $airlines->links('vendor.livewire.custom') }}
        @endslot
    </x-table-form>
